...
/// `"string"` slot, depending on the template field. Always holds Typst
/// markup (the AST walk has already converted LaTeX → Typst by the time
/// content lands here); the embedding-time choice is what to do with
/// inline markup characters (`_`, `*`).
///
/// Was previously an enum with `Plain` and `Typst` variants. `Plain`
/// was never constructed in production — every parser produced
/// `Content::Typst(...)`. The variant survived as dead code along with
/// the `match` arms; collapsing to a single `Typst` form mirrors what
/// the data actually is.
#[derive(Debug, Clone)]
pub(crate) enum Content {
    /// Typst-flavoured content (may contain markup like `_italic_`,
    /// `*bold*`, `[ref](key)`).
    Typst(String),
}
...
impl Content {
    ...
    pub fn as_content(&self) -> &str {
        let Content::Typst(s) = self;
        s
    }

    /// Render for embedding inside a Typst `"..."` string literal.
    /// Strips Typst inline markup (`_`, `*`) so it doesn't appear as
    /// literal characters inside a string slot, then escapes `\` and
    /// `"` for the surrounding string quotes.
    pub fn as_string_literal(&self) -> String {
        let Content::Typst(s) = self;
        let stripped = strip_inline_markup(s);
        escape_for_typst_string(&stripped)
    }
