...
fn superscript_star_then_slash_is_broken() {
    ...
    assert!(t.contains("h^* /") || t.contains("^* /"), "expected `^* /`; got:\n{t}");
}

#[test]
fn slash_then_star_is_broken() {
    let t = typ("$a/*b$");
    assert!(!t.contains("/*"), "`/*` (comment-open) must be broken in math; got:\n{t}");
}
...
fn star_not_adjacent_to_slash_unchanged() {
    ...
    let t = typ("$a^*b$");
    assert!(t.contains("a^*b"), "unrelated `^*` must be unchanged; got:\n{t}");
}
