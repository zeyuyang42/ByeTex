...
\end{document}";
    ...
    // The only safe pattern is `cases([...], [...])`  — last char before ` $` is `)`.
    // The broken pattern (before the fix) was `cases([...], [...]\) $`.
    let cases_start = t.find("cases(").expect("cases( not found");
    let after_cases = &t[cases_start..];
    // Find the end of the cases(...) call: the first `)` that closes `cases(` at depth 1.
    // Simple proxy: look for `]) $` (last row closes, then cases closes, then math ends).
    // The broken pattern has `]\) $` — last row closes then escaped `)` then math.
    assert!(
