...
//! into widescreen with `[aspectratio=169]`. ByeTex should honor that rather than
//! always forcing 16:9.
...
fn default_beamer_is_4_3() {
    ...
    assert!(t.contains("presentation-4-3"), "default beamer is 4:3; got:\n{t}");
    assert!(!t.contains("presentation-16-9"), "default is not widescreen");
}
...
fn aspectratio_169_is_widescreen() {
    ...
    assert!(t.contains("presentation-16-9"), "aspectratio=169 → 16:9; got:\n{t}");
}
...
fn aspectratio_43_is_standard() {
    ...
    assert!(t.contains("presentation-4-3"), "aspectratio=43 → 4:3; got:\n{t}");
}
