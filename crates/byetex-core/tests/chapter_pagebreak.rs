...
//! ON A NEW PAGE. ByeTex emits a weak `#pagebreak` before each top-level
...
//!
//! The assertions count `#pagebreak(` rather than an exact spelling: a twoside
//! `book` opens chapters RECTO, so the break is `#pagebreak(to: "odd", weak: true)`
//! there and a plain weak break elsewhere. Counting the literal made these fail
//! for a formatting change rather than a behavioural one.
//!
