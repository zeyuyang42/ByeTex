...
fn acl_renders_page_two_column_with_spanning_title() {
    ...
    let t = typ(src);
    assert!(t.contains(PAGE_2COL), "ACL must set page columns: 2;\noutput:\n{t}");
    assert!(t.contains(SPAN_TITLE), "ACL title must span both columns;\noutput:\n{t}");
    assert!(!t.contains("#columns(2)["), "must not use the old content-block wrap;\noutput:\n{t}");
    assert!(t.contains("Abstract"), "abstract must still render;\noutput:\n{t}");
}
...
fn ieeetran_variant_renders_page_two_column() {
    let src = "\\documentclass[journal,10pt]{IEEEtranTCOM}\n\\title{T}\n\
        \\begin{document}\\maketitle\n\\section{Intro}Body.\\end{document}";
    let t = typ(src);
    assert!(t.contains(PAGE_2COL), "IEEEtranTCOM must set page columns: 2;\noutput:\n{t}");
    assert!(t.contains(SPAN_TITLE), "IEEE title must span both columns;\noutput:\n{t}");
}
...
fn plain_article_stays_single_column() {
    let src = "\\documentclass{article}\n\\title{T}\n\
        \\begin{document}\\maketitle\n\\section{Intro}Body.\\end{document}";
    let t = typ(src);
    assert!(!t.contains(PAGE_2COL), "plain article must stay single-column;\noutput:\n{t}");
    assert!(!t.contains(SPAN_TITLE), "single-column title is not a spanning float;\noutput:\n{t}");
}
