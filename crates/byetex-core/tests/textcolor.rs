...
fn textcolor_never_leaks_raw_command() {
    ...
    assert!(
        ...
        "raw \\textcolor must not leak, got: {}",
        out.typst
    );
}
...
fn textcolor_in_math_emits_content() {
    ...
    assert!(
        ...
        "no \\textcolor leak into math, got: {}",
        out.typst
    );
}
...
fn definecolor_html_resolves() {
    ...
    assert!(
        t.contains("rgb(255, 136, 0)"),
        "custom HTML color must resolve; got:\n{t}"
    );
...
fn inline_rgb_model() {
    ...
    assert!(
        t.contains("#text(fill: rgb(") && t.contains("[y]"),
        "got:\n{t}"
    );
}
...
fn unknown_color_falls_back_to_plain_content() {
    ...
    assert!(
        !t.contains("#text(fill:"),
        "no fill for an unresolvable color; got:\n{t}"
    );
}
