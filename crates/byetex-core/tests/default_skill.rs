...
fn tikz_maps_to_tikz_skill() {
    assert_eq!(
        default_skill_for(&Category::Tikz),
...
fn bibliography_warning_suggests_bibliography_skill() {
    ...
    fs::write(
        dir.join("p.tex"),
        "\\documentclass{article}\\begin{document}x\\bibliography{NoSuchFile}\\end{document}",
    )
    .unwrap();
    let out = byetex_core::convert(
        &fs::read_to_string(dir.join("p.tex")).unwrap(),
        &byetex_core::ConvertOptions {
            source_name: Some("p.tex".into()),
    ...
    assert!(
        out.warnings
            .iter()
            .any(|w| w.suggested_skill.as_deref() == Some("byetex-bibliography")),
