...
//! These tests exercise byetex_core::project::plan_project + the CLI materializer.

use std::path::PathBuf;

fn core_fixture(rel: &str) -> PathBuf {
    // The fixtures live in byetex-core/tests/fixtures/ — reference them
    // via CARGO_MANIFEST_DIR from byetex-core's perspective.
    let workspace = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .unwrap();
    workspace
        .join("crates/byetex-core/tests/fixtures")
        .join(rel)
}
...
fn materialize_writes_expected_tree() {
    ...
    // replicate the logic here. Since we're in byetex-cli tests, we can reference
    // the module directly.
