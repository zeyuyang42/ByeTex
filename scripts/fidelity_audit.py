...
def corpus_display(path: Path, repo_root: Path = REPO_ROOT) -> str:
    """How the corpus location is written into the COMMITTED artifacts.

    Repo-relative whenever the corpus lives inside the checkout, which is the
    normal case. The absolute path would otherwise embed the developer's home
    directory (and username) in a public doc, and it regenerates on every run.
    A `BYETEX_CORPUS_DIR` pointing outside the repo degrades to the directory
...
def main() -> int:
    ...
    js = {
        "corpus_dir": corpus_display(CORPUS_DIR),
